            ...
            if let data = data, let response = response as? HTTPURLResponse {
                ...
                do {
                    ...
                    if ((result.object(forKey: "success") as? Bool ?? false) == true) {
                        self.Token = ((result as AnyObject).object(forKey: "result")! as AnyObject).object(forKey: "app_token") as? String ?? ""
                        let id : Int = ((result as AnyObject).object(forKey: "result")! as AnyObject).object(forKey: "track_id") as? Int ?? 0
                        
                        print("FreeBox::initializeToken : Token = \(self.Token) for id = \(id)")
                    }
                    else {
...
//
//
//func openSession(baseURL: URL, appToken: String, completion: @escaping (String?) -> Void) {
//    let urlChallenge = baseURL.appendingPathComponent("login/")
//    
...
//        
//        let password = hmacSHA1(key: appToken, message: challenge)
//        
//        let urlSession = baseURL.appendingPathComponent("login/session/")
//        let jsonLogin: [String: Any] = [
//            "app_id": FreeboxConfig.appId,
//            "password": password
//        ]
...
//            else { completion(nil); return }
//            
//            print("🔐 Session ouverte sur :", baseURL)
